...
describe('Firestore Security Rules for Chat Module', () => {
  ...
  it('should allow a user to read their own messages', () => {
    ...
    // }
    const regex = /match\s+\/users\/{userId}\/messages\/{messageId}\s*{[\s\S]*?allow\s+read\s*:\s*if\s+request\.auth\.uid\s+==\s+userId\s*;/;
  ...
  it('should allow a user to write to their own messages', () => {
    ...
    // }
    const regex = /match\s+\/users\/{userId}\/messages\/{messageId}\s*{[\s\S]*?allow\s+write\s*:\s*if\s+request\.auth\.uid\s+==\s+userId\s*;/;
    expect(rulesContent).toMatch(regex);
  });
  ...
  it('should NOT allow a user to read another user\'s messages', () => {
    ...
    const broadReadRule = /match\s+\/users\/{userId}\/messages\/{messageId}\s*{[\s\S]*?allow\s+read\s*:\s*if\s+request\.auth\s*!=\s*null\s*;/;
  ...
  it('should NOT allow a user to write to another user\'s messages', () => {
    ...
    const broadWriteRule = /match\s+\/users\/{userId}\/messages\/{messageId}\s*{[\s\S]*?allow\s+write\s*:\s*if\s+request\.auth\s*!=\s*null\s*;/;
