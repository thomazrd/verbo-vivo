 ...
 * 
 * INSTRUÇÕES:
 * 1. Execute `firebase init functions` na raiz do projeto.
 * 2. Escolha TypeScript.
 * 3. Copie todo o conteúdo deste arquivo.
 * 4. Cole o conteúdo no arquivo `functions/src/index.ts` que foi gerado.
 * 5. Execute `npm install` dentro da pasta `functions`.
 * 6. Execute `firebase deploy --only functions`.
 */
